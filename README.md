...
- **Fully Local & Open-Source:** Runs on your machine with Ollama and open-source models, with no API keys required.
...
Groq doesn't serve an embeddings API, so swapping to Groq only replaces the
...
retriever configuration). It is fast and runs entirely offline:
